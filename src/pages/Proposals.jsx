...
const Proposals = () => {
    ...
    // Onboarding refs
    const savedRFPsRef = useRef(null);
    const draftRFPsRef = useRef(null);
    const savedGrantsRef = useRef(null);
    const draftGrantsRef = useRef(null);

    // Create callback refs that register when elements are mounted
    const setSavedRFPsRef = useCallback((element) => {
        savedRFPsRef.current = element;
        if (element) {
            registerRef('saved-rfps', savedRFPsRef);
        }
    }, [registerRef]);

    const setDraftRFPsRef = useCallback((element) => {
        draftRFPsRef.current = element;
        if (element) {
            registerRef('draft-rfps', draftRFPsRef);
        }
    }, [registerRef]);

    const setSavedGrantsRef = useCallback((element) => {
        savedGrantsRef.current = element;
        if (element) {
            registerRef('saved-grants', savedGrantsRef);
        }
    }, [registerRef]);

    const setDraftGrantsRef = useCallback((element) => {
        draftGrantsRef.current = element;
        if (element) {
            registerRef('draft-grants', draftGrantsRef);
        }
    }, [registerRef]);

    // Fallback: Also register refs on mount in case callback refs don't fire
    useEffect(() => {
        if (savedRFPsRef.current) registerRef('saved-rfps', savedRFPsRef);
        if (draftRFPsRef.current) registerRef('draft-rfps', draftRFPsRef);
        if (savedGrantsRef.current) registerRef('saved-grants', savedGrantsRef);
        if (draftGrantsRef.current) registerRef('draft-grants', draftGrantsRef);
    }, [registerRef]);
    ...
    const handleSave = async (rfp) => {
        ...
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: err.response?.data?.message || 'Failed to save RFP',
                confirmButtonColor: '#2563EB',
            });
        } finally {
    ...
    const handleUnsave = async (rfpId) => {
        ...
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: err.response?.data?.message || 'Failed to unsave RFP',
                confirmButtonColor: '#2563EB',
            });
        } finally {
    ...
    const handleSaveGrant = async (grant) => {
        ...
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: err.response?.data?.message || 'Failed to save Grant',
                confirmButtonColor: '#2563EB',
            });
        } finally {
    ...
    const handleUnsaveGrant = async (grantId) => {
        ...
        try {
            ...
            Swal.fire({
                icon: 'error',
                title: 'Error',
                text: err.response?.data?.message || 'Failed to unsave Grant',
                confirmButtonColor: '#2563EB',
            });
        } finally {
            setSavingStates(prev => ({ ...prev, [grantId]: false }));
