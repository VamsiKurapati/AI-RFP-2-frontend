        ...
        <div key={category} className="mb-4">
          ...
          {values.map((value) => (
            <div key={value} className="flex items-center mb-2">
              <input
                type="checkbox"
                className="mr-2 text-[#4B5563] mt-1 w-3 h-3 cursor-pointer"
...
const Discover = () => {
  ...
  const discoverSavedRFPsRef = useRef(null);
  ...
  const setDiscoverSavedRFPsRef = useCallback((element) => {
    discoverSavedRFPsRef.current = element;
    if (element) {
      registerRef('discover-saved-rfps', discoverSavedRFPsRef);
    }
  }, [registerRef]);
  ...
  useEffect(() => {
    ...
    if (discoverSavedRFPsRef.current) registerRef('discover-saved-rfps', discoverSavedRFPsRef);
  }, [registerRef]);
  ...
  const handleSaveGrant = async (grant) => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Save Failed',
        text: err.response?.data?.message || err.response?.data?.error || 'Failed to save grant. Please try again.',
        confirmButtonColor: '#2563EB'
      });
      Swal.fire({
        icon: 'error',
  ...
  const handleUnsaveGrant = async (grantId) => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Unsave Failed',
        text: err.response?.data?.message || err.response?.data?.error || 'Failed to unsave grant. Please try again.',
        confirmButtonColor: '#2563EB'
      });
      Swal.fire({
        icon: 'error',
  ...
  const handleSave = async (rfp) => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Save Failed',
        text: err.response?.data?.message || err.response?.data?.error || 'Failed to save RFP. Please try again.',
        confirmButtonColor: '#2563EB'
      });
    } finally {
  ...
  const handleUnsave = async (rfpId) => {
    ...
    try {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Unsave Failed',
        text: err.response?.data?.message || err.response?.data?.error || 'Failed to unsave RFP. Please try again.',
        confirmButtonColor: '#2563EB'
      });
    } finally {
  ...
  const handleShare = (link) => {
    navigator.clipboard.writeText(link).then(() => {
      ...
      Swal.fire({
        icon: 'error',
        title: 'Copy Failed',
        text: err.response?.data?.message || err.response?.data?.error || 'Failed to copy link to clipboard',
        confirmButtonColor: '#2563EB'
      });
    });
  };
      ...
      <div className="mt-auto">
        <div className="flex justify-between items-center mt-4">
          <div className="flex gap-3 text-[#111827] text-lg">
            ...
            <MdOutlineShare onClick={() => handleShare(rfp.link)} className="cursor-pointer text-[#111827] transition-colors hover:text-[#2563EB]" title="Share this RFP link" />
          </div>
    ...
    <div className={`bg-white rounded-xl p-4 shadow-sm w-full mr-4 flex flex-col justify-between border border-[#E5E7EB] flex-shrink-0`}>
      ...
      <div className="flex justify-between items-center">
        ...
        <button
          ...
          aria-disabled={role === "Viewer" || rfp.generated}
          aria-label={role === "Viewer" ? "Viewer cannot generate proposal" : rfp.generated ? "Proposal already generated" : "Generate Proposal"}
          className={`text-[14px] font-medium transition-colors ${rfp.generated ? "text-[#9CA3AF] cursor-not-allowed" : "text-[#2563EB] hover:underline"}`}
          title={role === "Viewer" ? "Viewer cannot generate proposal" : rfp.generated ? "Proposal already generated" : "Generate proposal for this RFP"}
        >
          {rfp.generated ? "In Progress" : "Generate"}
        </button>
      </div>
        ...
        <div className="flex justify-between items-center gap-8 mb-2">
          ...
          <div className="flex top-2 gap-2 text-lg text-[#111827]">
            ...
            <MdOutlineShare
              ...
              className="cursor-pointer shrink-0 transition-colors hover:text-[#2563EB]"
              title="Share this grant link"
            />
          </div>
        </div>
      ...
      <div className="flex justify-between items-center">
        ...
        <div className="flex gap-2">
          <button
            ...
            className={`text-[14px] font-medium transition-colors ${grant.generated ? "text-[#9CA3AF] cursor-not-allowed" : "text-[#2563EB] hover:underline"}`}
            ...
            title={role === "Viewer" ? "Viewer cannot generate proposal" : grant.generated ? "Proposal already generated" : "Generate proposal for this grant"}
          >
      ...
      <>
        ...
        <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-xl p-6 w-96 max-w-[90vw] z-50 max-h-[90vh] overflow-y-auto custom-scrollbar">
          <div className="flex justify-between items-center mb-4">
            ...
            <button onClick={onClose} className="text-gray-500 hover:text-gray-700 transition-all duration-200 hover:scale-110 disabled:opacity-50 disabled:cursor-not-allowed" disabled={isUploading} title={isUploading ? "Please wait for upload to complete" : "Close upload modal"}>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-[#2563EB] transition-colors">
                <input
                  ...
                  required
                />
                <label htmlFor="document-upload" className="cursor-pointer" title={isUploading ? "Please wait for upload to complete" : "Click or drag files here to upload RFP document"}>
                ...
                <div className="mt-2 p-2 bg-green-50 border border-green-200 rounded-lg">
                  <div className="flex items-center justify-between">
                    ...
                    <button
                      ...
                      title={isUploading ? "Please wait for upload to complete" : "Remove uploaded file"}
                    >
            ...
            <div className="flex gap-3 pt-4">
              <button
                ...
                onClick={onClose}
                disabled={isUploading}
                className="flex-1 px-4 py-2 border rounded-lg hover:bg-gray-50 transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
              ...
              <button
                type="submit"
                disabled={isUploading}
                className="flex-1 px-4 py-2 bg-[#2563EB] text-white rounded-lg hover:bg-[#1d4ed8] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2"
                title={isUploading ? "Uploading file..." : "Upload RFP file"}
              >
                {isUploading ? (
                  <>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="border-2 border-dashed border-gray-300 rounded-lg p-4 text-center hover:border-[#2563EB] transition-colors">
                <input
                  ...
                  required
                />
                <label htmlFor="grant-document-upload" className="cursor-pointer" title={isUploading ? "Please wait for upload to complete" : "Click or drag files here to upload Grant document"}>
                  <div className="space-y-2">
                    <div className="text-gray-600">
                      <svg className="mx-auto h-12 w-12" stroke="currentColor" fill="none" viewBox="0 0 48 48">
                        <path d="M28 8H12a4 4 0 00-4 4v20m32-12v8m0 0v8a4 4 0 01-4 4H12a4 4 0 01-4-4v-4m32-4l-3.172-3.172a4 4 0 00-5.656 0L28 28M8 32l9.172-9.172a4 4 0 015.656 0L28 28m0 0l4 4m4-24h8m-4-4v8m-12 4h.02" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" />
            ...
            <div className="flex gap-3 pt-4">
              ...
              <button
                ...
                disabled={isUploading}
                className="flex-1 px-4 py-2 bg-[#2563EB] text-white rounded-lg hover:bg-[#1d4ed8] disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-[1.02] active:scale-[0.98] flex items-center justify-center gap-2"
                title={isUploading ? "Uploading file..." : "Upload Grant file"}
              >
                {isUploading ? (
                  <>
                    <div className="animate-spin rounded-full h-5 w-5 border-2 border-white border-t-transparent"></div>
  ...
  return (
    <>
      ...
      <div className={`${flag && !["Free", "Basic", "Pro", "Enterprise", "Custom Enterprise Plan"].includes(flag.plan_name) ? "h-screen" : "min-h-screen"} bg-[#FFFFFF]`}>
        ...
        <div className="fixed top-16 left-0 h-[calc(100vh-4rem)] z-40 hidden" />
        <LeftSidebar
        ...
        <main className="pt-20 px-8 md:px-12 py-6 ml-0">
          ...
          <div ref={setDiscoverHeaderRef} className="flex  justify-center mx-auto text-lg space-x-6 border-b border-gray-200 p-4">
            ...
            <button
              ...
              title="View Grant opportunities"
            >
              Grant's
            </button>
          </div>
          ...
          {activeTab === "rfp" && (
            <div className="mt-4">
              <div className="mb-8">
                <div className="flex flex-col md:flex-row gap-4 items-center">
                  ...
                  <div ref={setDiscoverActionsRef}>
                    <button
                      className="flex items-center gap-2 text-[16px] text-white bg-[#2563EB] px-4 py-3 rounded-md hover:cursor-pointer transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-[1.02] active:scale-[0.98]"
                      onClick={() => setUploadModalOpen(true)}
                      title="Upload an RFP document to generate proposals"
                    >
                ...
                {error && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4 text-center">
                    ...
                    <button
                      ...
                      title={retryCount > 0 ? `Retry loading proposals (${retryCount} attempts remaining)` : "Retry loading proposals"}
                    >
              ...
              <div ref={setDiscoverSavedRFPsRef}>
                ...
                {error && (
                  <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-4 text-center">
                    ...
                    <button
                      onClick={handleRetry}
                      className="mt-2 text-red-600 hover:text-red-800 underline transition-colors"
                      title={retryCount > 0 ? `Retry loading (${retryCount} attempts remaining)` : "Retry loading"}
                    >
                      {retryCount > 0 ? `Try again (${retryCount}/3)` : "Try again"}
                    </button>
                  </div>
              ...
              <div className="mb-8">
                <div className="flex flex-col md:flex-row gap-4 items-center">
                  ...
                  <button
                    ...
                    title="Upload a grant document to generate proposals"
                  >
