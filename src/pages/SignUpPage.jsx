          ...
          <div className="w-full md:w-1/2 max-w-lg">
                    ...
                    <div key={field}>
                      ...
                      <div className="relative">
                        <input
                          ...
                          placeholder={
                            field === "fullName"
                              ...
                              : field === "email"
                                ? "Enter Your Email Address (e.g., name@company.com)"
                                : field === "password"
                                  ? "Create Password (min. 8 characters with uppercase, lowercase, number, and special char)"
                                  : "Retype Your Password to Confirm"
                          }
                          value={form[field]}
                          onChange={handleChange}
                          disabled={isSubmitting || isSendingCode || isVerifyingCode}
                          className={`w-full p-3 bg-[#0000000F] rounded-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${errors[field] ? "border border-red-500" : ""} pr-12`}
                          title={
                            field === "fullName"
                              ? "Enter your full legal name"
                              : field === "email"
                                ? "Enter a valid email address for verification"
                                : field === "password"
                        ...
                        {field.includes("password") && (
                          <button
                            type="button"
                            className="absolute right-3 top-3 text-gray-600 hover:text-gray-800 transition-colors duration-200 disabled:opacity-50"
                    ...
                    <div className="mt-4 p-4 bg-gray-50 rounded-md">
                      <div className="flex items-center justify-between mb-3">
                        ...
                        <button
                          ...
                          disabled={isSendingCode || isEmailVerified || isSubmitting}
                          className={`px-4 py-2 text-sm font-medium rounded-md transition-all duration-200 ${isEmailVerified
                            ? "bg-green-100 text-green-700 cursor-not-allowed"
                            : isSendingCode
                              ? "bg-gray-100 text-gray-500 cursor-not-allowed"
                              : "bg-blue-600 text-white hover:bg-blue-700 shadow-md hover:shadow-lg"
                            }`}
                          title={isEmailVerified ? "Email is already verified" : isSendingCode ? "Sending verification code..." : "Send verification code to your email"}
                        >
                          ...
                          <div>
                            ...
                            <div className="flex gap-2">
                              <input
                                ...
                                maxLength="6"
                                disabled={isSendingCode || isVerifyingCode || isSubmitting}
                                className="flex-1 p-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed"
                                title="Enter the 6-digit verification code sent to your email"
                              />
                              <button
                                type="button"
                                onClick={verifyCode}
                                disabled={isVerifyingCode || !verificationCode.trim() || isSubmitting}
                                className={`px-4 py-2 text-sm font-medium rounded-md transition-all duration-200 shadow-md hover:shadow-lg ${isVerifyingCode || !verificationCode.trim()
                                  ? "bg-gray-100 text-gray-500 cursor-not-allowed"
                                  : "bg-green-600 text-white hover:bg-green-700"
                                  }`}
                                title={!verificationCode.trim() ? "Please enter verification code first" : isVerifyingCode ? "Verifying code..." : "Verify the code sent to your email"}
                              >
                            ...
                            {showResendButton && (
                              <div className="mt-2">
                                <button
                                  ...
                                  className={`px-4 py-2 text-sm font-medium rounded-md transition-all duration-200 shadow-md hover:shadow-lg ${isSendingCode
                                    ? "bg-gray-100 text-gray-500 cursor-not-allowed"
                                    : "bg-orange-600 text-white hover:bg-orange-700"
                                    }`}
                                  title={isSendingCode ? "Sending new verification code..." : "Resend verification code to your email"}
                                >
            ...
            {step === 2 && (
              <>
                <div className="space-y-4">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      className={`w-full p-3 bg-[#0000000F] rounded-md transition-colors duration-200 disabled:opacity-50 disabled:cursor-not-allowed ${errors.organization ? "border border-red-500" : ""}`}
                      title="Enter the name of your organization or company"
                    />
                ...
                <button
                  className="w-full mt-6 bg-[#2563EB] text-white py-3 px-4 rounded-md font-medium hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 transition-all duration-200 shadow-md hover:shadow-lg transform hover:scale-[1.02] active:scale-[0.98]"
                  onClick={handleSubmit}
                  disabled={isSubmitting}
                  title={isSubmitting ? "Creating your account..." : "Complete sign up and proceed to profile creation"}
                >
                  {isSubmitting ? (
                    <>
