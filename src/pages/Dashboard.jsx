...
const Dashboard = () => {
    ...
    const dashboardGrantProposalsRef = useRef(null);
    const dashboardDeletedProposalsRef = useRef(null);
    const dashboardDeletedGrantProposalsRef = useRef(null);
    ...
    const setDashboardGrantProposalsRef = useCallback((element) => {
        dashboardGrantProposalsRef.current = element;
        if (element) {
            registerRef('dashboard-grant-proposals', dashboardGrantProposalsRef);
        }
    }, [registerRef]);
    ...
    const setDashboardDeletedProposalsRef = useCallback((element) => {
        dashboardDeletedProposalsRef.current = element;
        if (element) {
            registerRef('dashboard-deleted-proposals', dashboardDeletedProposalsRef);
        }
    }, [registerRef]);

    const setDashboardDeletedGrantProposalsRef = useCallback((element) => {
        dashboardDeletedGrantProposalsRef.current = element;
        if (element) {
            registerRef('dashboard-deleted-grant-proposals', dashboardDeletedGrantProposalsRef);
        }
    }, [registerRef]);
    ...
    useEffect(() => {
        const registerAllRefs = () => {
            ...
            if (dashboardGrantProposalsRef.current) {
                registerRef('dashboard-grant-proposals', dashboardGrantProposalsRef);
            }
            if (dashboardCalendarRef.current) {
                registerRef('dashboard-calendar', dashboardCalendarRef);
            }
            if (dashboardDeletedProposalsRef.current) {
                registerRef('dashboard-deleted-proposals', dashboardDeletedProposalsRef);
            }
            if (dashboardDeletedGrantProposalsRef.current) {
                registerRef('dashboard-deleted-grant-proposals', dashboardDeletedGrantProposalsRef);
            }
        };
    ...
    useEffect(() => {
        if (!loading) {
            const registerAllRefs = () => {
                ...
                if (dashboardGrantProposalsRef.current) registerRef('dashboard-grant-proposals', dashboardGrantProposalsRef);
                if (dashboardCalendarRef.current) registerRef('dashboard-calendar', dashboardCalendarRef);
                if (dashboardDeletedProposalsRef.current) registerRef('dashboard-deleted-proposals', dashboardDeletedProposalsRef);
                if (dashboardDeletedGrantProposalsRef.current) registerRef('dashboard-deleted-grant-proposals', dashboardDeletedGrantProposalsRef);
            };
            ...
            >
                ...
                {sortedEvents.length > 0 && (
                    <>
                        ...
                        <div className={`absolute ${sortedEvents.length === 1 ? 'bottom-1' : 'bottom-6'} left-2 mb-1 flex flex-col items-start w-full`}>
                        ...
                        {sortedEvents.length > 1 && (
                            <div
                                className="absolute bottom-1 left-2 text-[10px] sm:text-[11px] text-[#2563EB] cursor-pointer font-medium hover:text-[#1d4ed8] transition-colors"
                                onClick={() => setOpenDropdownDate(isDropdownOpen ? null : dateKey)}
                                title={isDropdownOpen ? "Hide additional events" : `View ${sortedEvents.length - 1} more event(s) for this date`}
                            >
            ...
            <main className="w-full mx-auto py-8 px-4 md:px-12 mt-20">
                ...
                <div className="flex flex-row items-center justify-between mb-4 gap-2 mt-2">
                    <div className="flex relative w-full md:w-1/3 items-center gap-2">
                        <MdOutlineSearch className="absolute left-3 text-gray-500" title="Search proposals" />
                        <input
                            type="text"
                            placeholder="Search by proposal name, client, or status..."
                            className="border rounded px-3 py-2 w-full bg-white border-[#E5E7EB] text-[#9CA3AF] pl-10 focus:outline-none focus:ring-1 focus:ring-[#111827] transition-all duration-200"
                            value={search}
                            onChange={e => setSearch(e.target.value)}
                            title="Search RFP proposals by name, client name, or status"
                        />
                    </div>
                ...
                <div ref={setDashboardProposalsRef} className="bg-white rounded-lg shadow overflow-x-auto mb-8">
                                        ...
                                        <tr key={realIdx} className="border-t">
                                            {showDeleteOptions && (
                                                <td className="px-2 py-2">
                                                    <input
                                                        ...
                                                        title={selectedProposals.includes(realIdx) ? "Deselect this proposal" : "Select this proposal for deletion"}
                                                    />
                                                </td>
                                            )}
                                            ...
                                            <td className="px-4 py-2 font-semibold">
                                                <button
                                                    ...
                                                    className="text-[#2563EB] hover:underline cursor-pointer text-left transition-all duration-200 hover:text-[#1d4ed8]"
                                                    title="View RFP proposal details"
                                                >
                                            ...
                                            <td className="px-4 py-2">
                                                {editIdx === realIdx ? (
                                                    <input
                                                        ...
                                                        className="border border-[#111827] rounded px-2 py-1 text-[#111827] text-[16px] w-full bg-[#F3F4F6] focus:outline-none focus:ring-1 focus:ring-[#2563EB] disabled:opacity-50 disabled:cursor-not-allowed"
                                                        disabled={savingProposal[p._id]}
                                                        title="Select the deadline date for this proposal"
                                                    />
                                                ) : (
                                            ...
                                            <td className="px-4 py-2">
                                                {editIdx === realIdx ? (
                                                    <select className="w-full border border-[#111827] rounded px-3 py-2 text-[#111827] text-[16px] bg-[#F3F4F6] focus:outline-none focus:ring-2 focus:ring-[#2563EB] min-w-[140px] disabled:opacity-50 disabled:cursor-not-allowed" value={editForm.status} onChange={e => handleEditChange("status", e.target.value)} disabled={savingProposal[p._id]} title="Select the status for this proposal">
                                            ...
                                            <td className="px-4 py-2">
                                                {editIdx === realIdx ? (
                                                    <input
                                                        ...
                                                        className="border border-[#111827] rounded px-2 py-1 text-[#111827] text-[16px] w-full bg-[#F3F4F6] focus:outline-none focus:ring-1 focus:ring-[#2563EB] disabled:opacity-50 disabled:cursor-not-allowed"
                                                        disabled={savingProposal[p._id]}
                                                        title="Select the submission date for this proposal"
                                                    />
                                                ) : (
                                            ...
                                            <td className="px-4 py-2">
                                                {editIdx === realIdx ? (
                                                    <div className="flex items-center gap-2">
                                                        <button
                                                            className="flex items-center gap-1 text-[16px] text-[#2563EB] hover:text-white hover:bg-[#2563EB] rounded-md px-2 py-1 transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                                                            title={savingProposal[p._id] ? "Saving proposal..." : "Save proposal changes"}
                                                        ...
                                                        <button
                                                            className="flex items-center gap-1 text-[16px] text-[#111827] hover:text-white hover:bg-[#111827] rounded-md px-2 py-1 transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                                                            title={savingProposal[p._id] ? "Please wait..." : "Cancel editing proposal"}
                    ...
                    <div className="flex justify-end gap-2 my-2 px-4">
                        <button
                            className="px-2 py-1 border rounded disabled:opacity-50 disabled:cursor-not-allowed transition-colors hover:bg-gray-50"
                            onClick={() => setCurrentProposalPage(p => Math.max(1, p - 1))}
                            disabled={currentProposalPage === 1}
                            title={currentProposalPage === 1 ? "Already on first page" : "Go to previous page"}
                        >Prev</button>
                        {Array.from({ length: totalProposalPages }, (_, i) => (
                            <button
                                key={i}
                                className={`px-2 py-1 border rounded transition-colors hover:bg-gray-50 ${currentProposalPage === i + 1 ? 'bg-[#2563EB] text-white hover:bg-[#1D4ED8]' : ''}`}
                        ...
                        <button
                            ...
                            title={currentProposalPage === totalProposalPages ? "Already on last page" : "Go to next page"}
                        >Next</button>
                    </div>
                    {showDeleteOptions && (
                        <div className="flex gap-4 mt-4 justify-end mb-4 px-4">
                            <button
                                className="bg-red-500 text-white px-4 py-2 rounded disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 transition-all duration-200 hover:bg-red-600"
                                onClick={handleDeleteProposals}
                                disabled={deletingProposals}
                                title={deletingProposals ? "Deleting proposals..." : "Delete selected proposals"}
                            >
                                {deletingProposals ? (
                                    <>
                                        <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
                            ...
                            <button
                                className="bg-gray-300 text-gray-800 px-4 py-2 rounded disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 hover:bg-gray-400 shadow-md hover:shadow-lg transform hover:scale-[1.02] active:scale-[0.98]"
                                onClick={() => {
                                    setShowDeleteOptions(false);
                                    setSelectedProposals([]);
                                }}
                                disabled={deletingProposals}
                                title={deletingProposals ? "Please wait..." : "Cancel deletion"}
                            >
                                Cancel
                            </button>
                        </div>
                ...
                <div className="flex flex-row items-center justify-between mb-4 gap-2">
                    <div className="flex relative w-full md:w-1/3 items-center gap-2">
                        ...
                        <input
                            type="text"
                            placeholder="Search by grant title, agency, or status..."
                            className="border rounded px-3 py-2 w-full bg-white border-[#E5E7EB] text-[#9CA3AF] pl-10 focus:outline-none focus:ring-1 focus:ring-[#111827] transition-all duration-200"
                            value={grantSearch}
                            onChange={e => setGrantSearch(e.target.value)}
                            title="Search grant proposals by opportunity title, agency name, or status"
                        />
                    </div>
                    <div className="justify-end group flex gap-2">
                ...
                <div ref={setDashboardGrantProposalsRef} className="bg-white rounded-lg shadow overflow-x-auto mb-8">
                    <table className="min-w-full text-sm">
                        <thead>
                            <tr className="bg-[#F8FAFC]">
                                        ...
                                        <tr key={realIdx} className="border-t">
                                            {showGrantDeleteOptions && (
                                                <td className="px-2 py-2">
                                                    <input
                                                        ...
                                                        title={selectedGrantProposals.includes(realIdx) ? "Deselect this grant proposal" : "Select this grant proposal for deletion"}
                                                    />
                                                </td>
                                            )}
                                            ...
                                            <td className="px-4 py-2 font-semibold">
                                                <button
                                                    ...
                                                    className="text-[#2563EB] hover:underline cursor-pointer text-left transition-all duration-200 hover:text-[#1d4ed8]"
                                                    title="View Grant proposal details"
                                                >
                                                    {p.title || p.OPPORTUNITY_TITLE || 'Not Provided'}
                                                </button>
                                            </td>
                                            ...
                                            <td className="px-4 py-2">
                                                {editGrantIdx === realIdx ? (
                                                    <input
                                                        ...
                                                        className="border border-[#111827] rounded px-2 py-1 text-[#111827] text-[16px] w-full bg-[#F3F4F6] focus:outline-none focus:ring-1 focus:ring-[#2563EB] disabled:opacity-50 disabled:cursor-not-allowed"
                                                        disabled={savingGrantProposal[p._id]}
                                                        title="Select the close date for this grant proposal"
                                                    />
                                                ) : (
                                            ...
                                            <td className="px-4 py-2">
                                                {editGrantIdx === realIdx ? (
                                                    <input
                                                        ...
                                                        className="border border-[#111827] rounded px-2 py-1 text-[#111827] text-[16px] w-full bg-[#F3F4F6] focus:outline-none focus:ring-1 focus:ring-[#2563EB] disabled:opacity-50 disabled:cursor-not-allowed"
                                                        disabled={savingGrantProposal[p._id]}
                                                        title="Select the submission date for this grant proposal"
                                                    />
                                                ) : (
                                                    p.submittedAt ? new Date(p.submittedAt).toLocaleDateString('en-US', {
                                                        year: 'numeric',
                                            ...
                                            <td className="px-4 py-2">
                                                {editGrantIdx === realIdx ? (
                                                    <div className="flex items-center gap-2">
                                                        <button
                                                            className="flex items-center gap-1 text-[16px] text-[#2563EB] hover:text-white hover:bg-[#2563EB] rounded-md px-2 py-1 transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                                                            title={savingGrantProposal[p._id] ? "Saving grant proposal..." : "Save grant proposal changes"}
                                                        ...
                                                        <button
                                                            className="flex items-center gap-1 text-[16px] text-[#111827] hover:text-white hover:bg-[#111827] rounded-md px-2 py-1 transition-all duration-200 shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                                                            title={savingGrantProposal[p._id] ? "Please wait..." : "Cancel editing grant proposal"}
                    ...
                    <div className="flex justify-end gap-2 my-2 px-4">
                        <button
                            ...
                            title={currentGrantPage === 1 ? "Already on first page" : "Go to previous page"}
                        >Prev</button>
                        {Array.from({ length: totalGrantPages }, (_, i) => (
                            <button
                                key={i}
                                className={`px-2 py-1 border rounded transition-colors hover:bg-gray-50 ${currentGrantPage === i + 1 ? 'bg-[#2563EB] text-white hover:bg-[#1D4ED8]' : ''}`}
                        ...
                        <button
                            ...
                            title={currentGrantPage === totalGrantPages || paginatedGrantProposals.length === 0 ? "Already on last page or no proposals" : "Go to next page"}
                        >Next</button>
                    </div>
                    {showGrantDeleteOptions && (
                        <div className="flex gap-4 mt-4 justify-end mb-4 px-4">
                            <button
                                ...
                                title={deletingGrantProposals ? "Deleting grant proposals..." : paginatedGrantProposals.length === 0 ? "No grant proposals to delete" : "Delete selected grant proposals"}
                            >
                            ...
                            <button
                                ...
                                disabled={paginatedGrantProposals.length === 0 || deletingGrantProposals}
                                title={deletingGrantProposals ? "Please wait..." : "Cancel deletion"}
                            >
                                Cancel
                            </button>
                        </div>
                    ...
                    <div className="flex flex-col xs:flex-row items-center justify-between mb-4">
                        <div className="flex gap-2 mb-4">
                            <select
                                ...
                                className="bg-[#F3F4F6] border rounded-md px-2 py-1 text-[#111827] text-xs sm:text-base transition-colors hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-[#2563EB]"
                                title="Select month to view calendar"
                            >
                            ...
                            <select
                                ...
                                className="bg-[#F3F4F6] border rounded-md px-2 py-1 text-[#111827] text-xs sm:text-base transition-colors hover:bg-gray-200 focus:outline-none focus:ring-2 focus:ring-[#2563EB]"
                                title="Select year to view calendar"
                            >
                                {yearOptions.map(y => (
                ...
                <div ref={setDashboardDeletedProposalsRef} className="bg-white rounded-lg overflow-x-auto shadow mb-8">
                    <table className="min-w-full text-sm ">
                        <thead>
                            <tr className="bg-[#F8FAFC]">
                                <th className="px-4 py-2 text-left font-medium">Proposal Name</th>
                        ...
                        <tbody>
                                ...
                                return (
                                    <tr key={absoluteIdx} className="border-t">
                                        ...
                                        <td className="px-4 py-2">
                                            <div className="flex items-center gap-2">
                                                <button
                                                    className="text-[#2563EB] disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1 transition-colors hover:text-[#1d4ed8]"
                                                    title={restoringProposal[absoluteIdx] ? "Restoring proposal..." : "Restore this deleted proposal"}
                                                ...
                                                <button
                                                    className={`${role === "Viewer" ? "text-[#9CA3AF] cursor-not-allowed opacity-50" : "text-[#2563EB]"} disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1 transition-colors hover:text-[#1d4ed8]`}
                                                    title={role === "Viewer" ? "Viewer cannot delete permanently" : deletingPermanently[absoluteIdx] ? "Deleting permanently..." : "Delete this proposal permanently (cannot be undone)"}
                            ...
                            {paginatedDeletedProposals.length === 0 && (
                                <tr>
                                    <td colSpan={7} className="text-center py-4">No deleted proposals founds.</td>
                                </tr>
                            )}
                        </tbody>
                    </table>
                    ...
                    <div className="flex justify-end gap-2 my-2 px-4">
                        <button
                            ...
                            title={currentDeletedPage === 1 ? "Already on first page" : "Go to previous page"}
                        >Prev</button>
                        {Array.from({ length: totalDeletedPages }, (_, i) => (
                            <button
                                key={i}
                                className={`px-2 py-1 border rounded transition-colors hover:bg-gray-50 ${currentDeletedPage === i + 1 ? 'bg-[#2563EB] text-white hover:bg-[#1d4ed8]' : ''}`}
                                value={i + 1}
                        ...
                        <button
                            ...
                            title={currentDeletedPage === totalDeletedPages ? "Already on last page" : "Go to next page"}
                        >Next</button>
                    </div>
                </div>
                ...
                <div ref={setDashboardDeletedGrantProposalsRef} className="bg-white rounded-lg overflow-x-auto shadow mb-8">
                    <table className="min-w-full text-sm ">
                        <thead>
                            <tr className="bg-[#F8FAFC]">
                                <th className="px-4 py-2 text-left font-medium">Opportunity Title</th>
                                    ...
                                    <tr key={absoluteIdx} className="border-t">
                                        ...
                                        <td className="px-4 py-2">
                                            <div className="flex items-center gap-2">
                                                <button
                                                    className="text-[#2563EB] disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1 transition-colors hover:text-[#1d4ed8]"
                                                    title={restoringGrantProposal[absoluteIdx] ? "Restoring grant proposal..." : "Restore this deleted grant proposal"}
                                                ...
                                                <button
                                                    className={`${role === "Viewer" ? "text-[#9CA3AF] cursor-not-allowed opacity-50" : "text-[#2563EB]"} disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-1 transition-colors hover:text-[#1d4ed8]`}
                                                    title={role === "Viewer" ? "Viewer cannot delete permanently" : deletingGrantPermanently[absoluteIdx] ? "Deleting permanently..." : "Delete this grant proposal permanently (cannot be undone)"}
                    ...
                    <div className="flex justify-end gap-2 my-2 px-4">
                        <button
                            ...
                            title={currentDeletedGrantPage === 1 ? "Already on first page" : "Go to previous page"}
                        >Prev</button>
                        {Array.from({ length: totalDeletedGrantPages }, (_, i) => (
                            <button
                                key={i}
                                className={`px-2 py-1 border rounded transition-colors hover:bg-gray-50 ${currentDeletedGrantPage === i + 1 ? 'bg-[#2563EB] text-white hover:bg-[#1d4ed8]' : ''}`}
                        ...
                        <button
                            ...
                            title={currentDeletedGrantPage === totalDeletedGrantPages ? "Already on last page" : "Go to next page"}
                        >Next</button>
                    </div>
                </div>
            </main>
