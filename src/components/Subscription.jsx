...
export default function Subscription({ plan }) {
  ...
  const getPlanSection = (planName) => {
    ...
    return (
      <div
        className={`mx-auto border rounded-2xl p-4 w-full max-w-[340px] h-[500px] shadow-md relative transition-transform hover:scale-105 flex flex-col bg-white mb-[40px] z-[70] ${popularPlan === planName ? "border-blue-500" : "border-gray-300"
          }`}
      >
        {popularPlan === planName && (
          <div className="absolute -top-3 left-1/2 transform -translate-x-1/2 bg-gradient-to-r from-[#2F3349] to-[#717AAF] text-white text-xs px-3 py-1 rounded-full z-[70] shadow-md">
            Most Popular
          </div>
        )}
