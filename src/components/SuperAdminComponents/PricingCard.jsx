        ...
        >
            ...
            <div className="flex justify-center mb-4">
                <div className="inline-flex items-center bg-gray-100 rounded-full p-1">
                    <button
                        ...
                        disabled={!isEditing}
                        title={isEditing ? "Switch to monthly pricing" : "Monthly pricing"}
                    >
                        Monthly
                    </button>
                    <button
                        ...
                        disabled={!isEditing}
                        title={isEditing ? "Switch to yearly pricing" : "Yearly pricing"}
                    >
                        Yearly
                    </button>
                </div>
            ...
            {!isEditing ? (
                ...
                <input
                    ...
                    disabled={loading}
                    title="Enter the new subscription plan price"
                />
            )}
            ...
            {!isEditing ? (
                <button
                    ...
                    onClick={startEdit}
                    title="Edit subscription plan price"
                >
                    Edit
                </button>
            ) : (
                <div className="flex gap-2 mt-auto">
                    <button
                        ...
                        onClick={cancelEdit}
                        disabled={loading}
                        title={loading ? "Saving, please wait..." : "Cancel editing"}
                    >
                        Cancel
                    </button>
                    <button
                        className="w-1/2 py-2 rounded-lg bg-gradient-to-b from-[#6C63FF] to-[#3F73BD] text-white font-medium shadow transition-all duration-200 hover:shadow-lg transform hover:scale-[1.02] active:scale-[0.98] disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                        onClick={saveEdit}
                        disabled={loading}
                        title={loading ? "Saving price..." : "Save price changes"}
                    >
                        {loading ? (
                            <>
